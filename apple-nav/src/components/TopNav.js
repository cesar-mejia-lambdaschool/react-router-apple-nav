...
class TopNav extends React.Component {
  ...
  checkDesktop = () => {
    ...
    if (window.innerWidth > 767) return this.setState({isDesktop: true, isMenuActive: false})
  ...
  renderCollapsedNav = () => {
    ...
    >
      <nav>
      <i onClick={this.handleClick} id='hamburger' className="fas fa-bars fa-2x"></i>
          <Link id='apple-link' to='/'><i id='apple' className='fab fa-apple fa-2x' /></Link>
  ...
  renderExpandedNav = () => {
    return (
      <header id='header-expanded'>
        <i id='hamburger-expanded' onClick={this.handleClick}  class="fas fa-times fa-2x"></i>
      <nav className='nav-expanded'>
      <Link id='apple-link-expanded' to='/'><i id='expanded-apple' className='fab fa-apple fa-2x' /></Link>
      </nav>
<div className='expanded-menu'>
<input className='search-input' type='text' placeholder='Search apple.com'/>
      <ul>
       <li>
       Mac
       </li>
  ...
  render() {
    ...
    return (
      <Fragment>{isMenuActive && !isDesktop ? this.renderExpandedNav() : this.renderCollapsedNav()}</Fragment>
    )
  }
}
